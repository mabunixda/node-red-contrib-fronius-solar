...
describe('Fronius Node', function () {
    ...
    it('should be loaded', function (done) {
        const flow = [
            { id: 'n1', type: 'fronius-inverter', name: 'test name' }
        ];
    ...
    it('should handle missing inverter config gracefully', function (done) {
        const flow = [
            { id: 'n2', type: 'fronius-control', name: 'test control', inverter: null }
        ];
        helper.load(froniusNode, flow, function () {
    ...
    it('should call processCommand and handle inverter query', function (done) {
        const fakeData = { Head: { Status: { Code: 0 } }, Body: { Data: { value: 42 } } };
        froniusApiMock.GetInverterRealtimeData.resolves(fakeData);
        const flow = [
            { id: 'n1', type: 'fronius-inverter', name: 'test inverter', host: 'localhost', port: 80, apiversion: 1 },
            { id: 'n2', type: 'fronius-control', name: 'test control', inverter: 'n1', querytype: 'inverter', deviceid: 1, wires: [['n3']] },
            { id: 'n3', type: 'helper' }
        ];
        helper.load(froniusNode, flow, function () {
            const n2 = helper.getNode('n2');
    ...
    it('should call processCommand and handle components query', function (done) {
        const fakeData = { Head: { Status: { Code: 0 } }, Body: { Data: { comp: 123 } } };
        froniusApiMock.GetComponentsData.resolves(fakeData);
        const flow = [
            { id: 'n1', type: 'fronius-inverter', name: 'test inverter', host: 'localhost', port: 80, apiversion: 1 },
            { id: 'n2', type: 'fronius-control', name: 'test control', inverter: 'n1', querytype: 'components', deviceid: 1, wires: [['n3']] },
            { id: 'n3', type: 'helper' }
        ];
        helper.load(froniusNode, flow, function () {
            const n2 = helper.getNode('n2');
    ...
    it('should call processCommand and handle powerflow query', function (done) {
        const fakeData = { Head: { Status: { Code: 0 } }, Body: { Data: { pf: 'ok' } } };
        froniusApiMock.GetPowerFlowRealtimeData.resolves(fakeData);
        const flow = [
            { id: 'n1', type: 'fronius-inverter', name: 'test inverter', host: 'localhost', port: 80, apiversion: 1 },
            { id: 'n2', type: 'fronius-control', name: 'test control', inverter: 'n1', querytype: 'powerflow', deviceid: 1, wires: [['n3']] },
            { id: 'n3', type: 'helper' }
        ];
        helper.load(froniusNode, flow, function () {
            const n2 = helper.getNode('n2');
    ...
    it('should call processCommand and handle storage query', function (done) {
        const fakeData = { Head: { Status: { Code: 0 } }, Body: { Data: { storage: true } } };
        froniusApiMock.GetStorageRealtimeData.resolves(fakeData);
        const flow = [
            { id: 'n1', type: 'fronius-inverter', name: 'test inverter', host: 'localhost', port: 80, apiversion: 1 },
            { id: 'n2', type: 'fronius-control', name: 'test control', inverter: 'n1', querytype: 'storage', deviceid: 1, wires: [['n3']] },
            { id: 'n3', type: 'helper' }
        ];
        helper.load(froniusNode, flow, function () {
            const n2 = helper.getNode('n2');
    ...
    it('should call processCommand and handle powermeter query', function (done) {
        const fakeData = { Head: { Status: { Code: 0 } }, Body: { Data: { meter: 999 } } };
        froniusApiMock.GetMeterRealtimeData.resolves(fakeData);
        const flow = [
            { id: 'n1', type: 'fronius-inverter', name: 'test inverter', host: 'localhost', port: 80, apiversion: 1 },
            { id: 'n2', type: 'fronius-control', name: 'test control', inverter: 'n1', querytype: 'powermeter', deviceid: 1, wires: [['n3']] },
            { id: 'n3', type: 'helper' }
        ];
        helper.load(froniusNode, flow, function () {
            const n2 = helper.getNode('n2');
    ...
    it('should set status to orange on invalid head', function (done) {
        const fakeData = { Head: { Status: { Code: 1, UserMessage: 'Invalid' } }, Body: { Data: {} } };
        froniusApiMock.GetInverterRealtimeData.resolves(fakeData);
        const flow = [
            { id: 'n1', type: 'fronius-inverter', name: 'test inverter', host: 'localhost', port: 80, apiversion: 1 },
            { id: 'n2', type: 'fronius-control', name: 'test control', inverter: 'n1', querytype: 'inverter', deviceid: 1, wires: [['n3']] },
        ...
        helper.load(froniusNode, flow, function () {
            ...
            sinon.stub(n2, 'status').callsFake(function (status) {
                status.should.have.property('fill', 'orange');
                status.should.have.property('text', 'Invalid');
                done();
            });
            n2.receive({});
        });
    ...
    it('should set status to red on fronius error', function (done) {
        froniusApiMock.GetInverterRealtimeData.rejects('error!');
        const flow = [
            { id: 'n1', type: 'fronius-inverter', name: 'test inverter', host: 'localhost', port: 80, apiversion: 1 },
            { id: 'n2', type: 'fronius-control', name: 'test control', inverter: 'n1', querytype: 'inverter', deviceid: 1, wires: [['n3']] },
            { id: 'n3', type: 'helper' }
    ...
    // More tests for processCommand, error handling, etc. can be added here
});
